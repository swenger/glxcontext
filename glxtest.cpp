...
/* to allow remote X server connections:
 *
 * http://ubuntuforums.org/showthread.php?t=1590100
 *
 * Edit /etc/X11/xinit/xserverrc: Remove the "-nolisten tcp"
 * Edit /etc/gdm/gdm.conf (or schema): Comment out the "DisallowTCP=true" so that it doesn't add the -nolisten back.
 * Restart X
 *
 * To check: ps -ef | grep X should not show X with -nolisten
 */
